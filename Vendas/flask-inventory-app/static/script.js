...
function filtrarProdutos() {
  ...
  produtos.forEach((prod, i) => {
    if (prod.quantity > 0 && (busca === "" || prod.title.toLowerCase().includes(busca))) {
      const tr = document.createElement("tr");
      tr.innerHTML = `
        <td>${prod.title}</td>
        <td>R$ ${prod.price.toFixed(2)}</td>
...
function adicionarAvulso() {
  const title = document.getElementById("avulso-title").value.trim();
  ...
  if (!title || isNaN(price) || isNaN(quantity) || quantity <= 0) {
  ...
  document.getElementById("avulso-title").value = "";
  document.getElementById("avulso-price").value = "";
  document.getElementById("avulso-quantity").value = "1";
